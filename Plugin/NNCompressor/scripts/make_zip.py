...
def  main():
  ...
  installer = "\installer\NNCompressor Installer.exe"
   
  if demo:
    installer = "\installer\NNCompressor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NNCompressor manual.pdf" 
  ]
  ...
  ZIPNAME = "NNCompressor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NNCompressor-v" + FULLVERSIONSTR + "-win-demo.zip"
