...
def  main():
  ...
  installer = "\installer\NNComp Installer.exe"
   
  if demo:
    installer = "\installer\NNComp Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NNComp manual.pdf" 
  ]
  ...
  ZIPNAME = "NNComp-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NNComp-v" + FULLVERSIONSTR + "-win-demo.zip"
